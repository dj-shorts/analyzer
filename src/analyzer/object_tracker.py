...
class ObjectTracker:
    ...
    def __init__(self, config: Config):
        ...
        self.confidence_threshold = getattr(
            config, "tracking_confidence_threshold", 0.5
        )
        ...
        logger.info(
            f"ObjectTracker initialized with smoothness={self.tracking_smoothness}, "
    ...
    def analyze_video_tracking(self, video_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Video properties: {fps:.2f} fps, {total_frames} frames, {width}x{height}"
            )
            ...
            logger.info(
                f"Object tracking: sampling every {frame_interval} frames (~{expected_processed_frames} frames to process)"
            )
            ...
            with tqdm(
                ...
                while True:
                    ...
                    if frame_count % frame_interval == 0:
                        ...
                        crop_center, confidence = self._process_frame(
                            frame, width, height
                        )
                        ...
                        if self.debug_tracking:
                            debug_frame = self._create_debug_frame(
                                frame, crop_center, confidence, width, height
                            )
                        ...
                        pbar.set_postfix(
                            {
                                "objects": len(
                                    [c for c in confidence_scores[-5:] if c > 0.5]
                                ),
            ...
            logger.info(
                f"Object tracking completed: {processed_count} frames processed in {processing_time:.2f}s"
            )
            ...
            smoothed_positions = self._smooth_crop_positions(
                crop_positions, confidence_scores
            )
            ...
            avg_confidence = (
                float(np.mean(confidence_scores)) if len(confidence_scores) else 0.0
            )
            detection_rate = (
                float(detections_count / processed_count) if processed_count else 0.0
            )
            metrics = {
    ...
    def _process_frame(
        ...
        try:
            ...
            if (
                ...
                and self.tracking_lost_frames < self.max_lost_frames
            ):
                ...
                return (
                    center_x,
                    center_y,
                ), 0.3  # Lower confidence for predicted position
    ...
    def _smooth_crop_positions(
        self, positions: np.ndarray, confidences: np.ndarray
    ) -> np.ndarray:
        """
        Smooth crop positions to avoid jittery movements.
    ...
    def _create_debug_frame(
        ...
        )  # Green if confident, red if low confidence
        ...
        # Draw crosshairs
        cv2.line(
            debug_frame, (center_x - 20, center_y), (center_x + 20, center_y), color, 2
        )
        cv2.line(
            debug_frame, (center_x, center_y - 20), (center_x, center_y + 20), color, 2
        )
        ...
        cv2.putText(
            debug_frame, position_text, (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2
        )

        # Add crop dimensions text
        crop_text = f"Crop: {crop_width}x{crop_height}"
        cv2.putText(
            debug_frame, crop_text, (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2
        )

        return debug_frame

    def _save_debug_video(
        self, video_path: Path, fps: float, width: int, height: int
    ) -> str | None:
        """
        ...
        try:
            ...
            debug_video_path = (
                video_path.parent / f"{video_path.stem}_debug_tracking.mp4"
            )
            ...
            if not out.isOpened():
                logger.error(
                    f"Failed to create debug video writer for {debug_video_path}"
                )
                return None
    ...
    def _create_fallback_tracking_data(self) -> dict[str, Any]:
        ...
        return {
            "crop_positions": np.array(
                [[0, 0]]
            ),  # Will be replaced with actual video center
    ...
    def interpolate_to_export_timeline(
        self, tracking_data: dict[str, Any], export_times: np.ndarray
    ) -> list[tuple[int, int]]:
        """
        ...
        interpolated_positions = list(
            zip(interpolated_x.astype(int), interpolated_y.astype(int), strict=True)
        )
