...
class MotionDetector:
    ...
    def _smooth_motion_scores(self, scores: np.ndarray, times: np.ndarray) -> np.ndarray:
    ...
    def interpolate_to_audio_timeline(self, motion_data: Dict[str, Any], audio_timeline: np.ndarray) -> np.ndarray:
        """
        ...
        if len(motion_times) == 0 or len(motion_scores) == 0:
            logger.warning("No motion data available for interpolation, returning zeros.")
            return np.zeros_like(audio_timeline)
        
        # Ensure motion_times is monotonically increasing
        if not np.all(np.diff(motion_times) >= 0):
            logger.warning("Motion times are not monotonically increasing. Sorting them.")
        ...
        interpolated_motion_scores = np.interp(audio_timeline, motion_times, motion_scores)
        
        return interpolated_motion_scores
    
    def combine_audio_and_motion_scores(self, audio_scores: np.ndarray, motion_scores: np.ndarray) -> np.ndarray:
        """
