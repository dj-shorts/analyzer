...
class DynamicCropper:
    ...
    def __init__(self, config: Config):
        ...
        logger.info(
            f"DynamicCropper initialized with smoothness={self.tracking_smoothness}"
        )
    ...
    def generate_crop_filter(
        ...
        if len(tracking_positions) == 1:
            ...
            crop_y = max(
                0, min(center_y - crop_height // 2, video_height - crop_height)
            )
    ...
    def _generate_dynamic_filter(
        ...
        time_interval = (
            duration / (len(tracking_positions) - 1)
            if len(tracking_positions) > 1
            else 0
        )
        ...
        x_expr = self._generate_position_expression(
            crop_x_positions, time_interval, start_time
        )

        # Generate FFmpeg expression for y position
        y_expr = self._generate_position_expression(
            crop_y_positions, time_interval, start_time
        )
    ...
    def _generate_position_expression(
        self, positions: list[int], time_interval: float, start_time: float
    ) -> str:
        """
        ...
        if position_variance > 20:  # Significant movement detected
            ...
            if len(positions) >= 2:
                ...
                logger.debug(
                    f"Using linear interpolation: {start_pos} -> {end_pos} over {total_duration:.3f}s"
                )
                return expr
    ...
    def calculate_crop_dimensions(
        self, video_width: int, video_height: int, target_format: str
    ) -> tuple[int, int]:
        """
        ...
        logger.debug(
            f"Calculated crop dimensions for {target_format}: {crop_width}x{crop_height}"
        )
    ...
    def validate_crop_positions(
        ...
        for center_x, center_y in positions:
            ...
            clamped_y = max(
                crop_height // 2, min(center_y, video_height - crop_height // 2)
            )
