...
class PeakPicker:
    ...
    def _incorporate_seeds(
        ...
        for seed_time in self.config.seed_timestamps:
            ...
            window_start = max(
                0, seed_frame_idx - int(seed_window * len(time_axis) / time_axis[-1])
            )
            window_end = min(
                len(novelty_scores),
                seed_frame_idx + int(seed_window * len(time_axis) / time_axis[-1]),
            )
        ...
        for _i, (peak_time, peak_score) in enumerate(
            zip(peak_times, peak_scores, strict=True)
        ):
