...
class AnalysisMetrics:
    ...
    def start_stage(self, stage: AnalysisStage) -> None:
        ...
        self.stage_timings[stage] = StageTiming(stage=stage, start_time=time.time())
    ...
    def finish_stage(self, stage: AnalysisStage) -> None:
        ...
        if stage in self.stage_timings:
            ...
            logger.debug(
                f"Finished stage: {stage.value} in {self.stage_timings[stage].duration:.3f}s"
            )
    ...
    def get_stage_duration(self, stage: AnalysisStage) -> float:
        ...
        if (
            stage in self.stage_timings
            and self.stage_timings[stage].duration is not None
        ):
            ...
            "timings": {
                ...
                "stages": {
                    stage.value: {
                        ...
                        "start_time": self.stage_timings[stage].start_time
                        if stage in self.stage_timings
                        else None,
                        "end_time": self.stage_timings[stage].end_time
                        if stage in self.stage_timings
                        else None,
                    }
                    for stage in AnalysisStage
...
class MetricsCollector:
    ...
    def set_audio_metrics(
        self, duration: float, sample_rate: int, bytes_count: int
    ) -> None:
    ...
    def set_video_metrics(
        self, duration: float, bytes_count: int, width: int, height: int
    ) -> None:
...
def format_prometheus_metrics(metrics: AnalysisMetrics) -> str:
    ...
    lines.append("# HELP job_duration_seconds Duration of analysis job in seconds")
    lines.append("# TYPE job_duration_seconds counter")
    ...
    # Novelty metrics
    lines.append("# HELP novelty_peaks_count Number of novelty peaks detected")
    lines.append("# TYPE novelty_peaks_count gauge")
    lines.append(f"novelty_peaks_count {metrics.novelty_peaks_count}")

    lines.append("# HELP novelty_frames_count Number of novelty frames processed")
    lines.append("# TYPE novelty_frames_count gauge")
    lines.append(f"novelty_frames_count {metrics.novelty_frames_count}")

    # Audio metrics
    lines.append("# HELP audio_duration_seconds Audio duration in seconds")
    lines.append("# TYPE audio_duration_seconds gauge")
    lines.append(f"audio_duration_seconds {metrics.audio_duration}")

    lines.append("# HELP audio_bytes Audio data size in bytes")
    lines.append("# TYPE audio_bytes gauge")
    lines.append(f"audio_bytes {metrics.audio_bytes}")

    # Video metrics
    lines.append("# HELP video_duration_seconds Video duration in seconds")
    lines.append("# TYPE video_duration_seconds gauge")
    lines.append(f"video_duration_seconds {metrics.video_duration}")

    lines.append("# HELP video_bytes Video file size in bytes")
    lines.append("# TYPE video_bytes gauge")
    lines.append(f"video_bytes {metrics.video_bytes}")

    # Processing metrics
    lines.append("# HELP clips_generated Number of clips generated")
    lines.append("# TYPE clips_generated gauge")
    lines.append(f"clips_generated {metrics.clips_generated}")

    lines.append("# HELP memory_peak_mb Peak memory usage in MB")
    lines.append("# TYPE memory_peak_mb gauge")
    lines.append(f"memory_peak_mb {metrics.memory_peak_mb}")

    # Configuration metrics
    lines.append("# HELP clips_requested Number of clips requested")
    lines.append("# TYPE clips_requested gauge")
    lines.append(f"clips_requested {metrics.clips_requested}")

    lines.append("# HELP with_motion_enabled Motion analysis enabled flag")
    lines.append("# TYPE with_motion_enabled gauge")
    lines.append(f"with_motion_enabled {1 if metrics.with_motion else 0}")

    lines.append("# HELP align_to_beat_enabled Beat alignment enabled flag")
    lines.append("# TYPE align_to_beat_enabled gauge")
