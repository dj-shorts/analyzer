...
class PeopleDetector:
    ...
    def detect_people_in_video_segment(
        self, video_path: Path, start_time: float, duration: float
    ) -> dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Detecting people in video segment: {start_time:.2f}s - {start_time + duration:.2f}s"
        )
            ...
            while frame_count <= end_frame:
                ...
                if (frame_count - start_frame) % sample_interval == 0:
                    ...
                    if (
                        ...
                        and detection_result["center_x"] is not None
                    ):
    ...
    def calculate_crop_window(
        ...
        if target_aspect > input_aspect:
            ...
            crop_x = max(
                0, min(input_width - crop_width, int(center_x - crop_width / 2))
            )
            crop_y = 0
        else:
            ...
            crop_y = max(
                0,
                min(
                    input_height - crop_height, int(input_height / 2 - crop_height / 2)
                ),
            )
