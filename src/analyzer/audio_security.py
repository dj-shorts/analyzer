...
def validate_audio_file(file_path: Path) -> bool:
    ...
    logger.info(
        f"Audio file validation passed: {file_path.name} ({file_size_mb:.1f}MB)"
    )
    return True
...
def safe_load_audio(
    ...
    try:
        ...
        audio_data, sample_rate = librosa.load(
            str(file_path), sr=sr, mono=mono, duration=duration, offset=0.0
        )
        ...
        if not np.all(np.isfinite(audio_data)):
            raise ValueError(
                "Audio file contains invalid data (NaN or infinite values)"
            )
        ...
        if actual_duration > MAX_AUDIO_DURATION_SECONDS:
            raise ValueError(
                f"Audio duration too long: {actual_duration:.1f}s > {MAX_AUDIO_DURATION_SECONDS}s"
            )

        logger.info(
            f"Audio loaded safely: {len(audio_data)} samples, {sample_rate}Hz, {actual_duration:.1f}s"
        )
...
def safe_resample_audio(audio: np.ndarray, orig_sr: int, target_sr: int) -> np.ndarray:
    """
    ...
    if target_sr > MAX_SAMPLE_RATE:
        raise ValueError(
            f"Target sample rate too high: {target_sr} > {MAX_SAMPLE_RATE}"
        )
